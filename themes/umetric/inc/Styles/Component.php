<?php
/**
 * Umetric\Utility\Styles\Component class
 *
 * @package umetric
 */

namespace Umetric\Utility\Styles;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'umetric-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = umetric()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'umetric-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'umetric' ), $handle ) ), 'Umetric 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'umetric_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! umetric()->is_amp();
		 ...
		 */
		return apply_filters( 'umetric_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'umetric-button'     => array(
				'file'   => 'button.min.css',
				'global' => true,
			),
			'umetric-comments'   => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'umetric-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return umetric()->is_primary_sidebar_active();
				},
				'global' => true,
			),
			'umetric-widgets'    => array(
		 ...
		 */
		$css_files = apply_filters( 'umetric_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			'Outfit' => array( '300' ,'400' ,'500', '600','700' ,'800', '900' ),
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'umetric_google_fonts', $google_fonts );
