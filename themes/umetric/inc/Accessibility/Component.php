...
/**
 * Umetric\Utility\Accessibility\Component class
 *
 * @package umetric
 */

namespace Umetric\Utility\Accessibility;
...
class Component implements Component_Interface
{
	...
	public function action_enqueue_navigation_script()
	{
		...
		wp_script_add_data('umetric-navigation', 'async', true);
		wp_script_add_data('umetric-navigation', 'precache', true);
		wp_localize_script(
			'umetric-navigation',
			'umetricScreenReaderText',
			array(
				'expand'   => __('Expand child menu', 'umetric'),
				'collapse' => __('Collapse child menu', 'umetric'),
			)
		);
	}
	...
	public function action_print_skip_link_focus_fix()
	{
		...
		wp_register_script('umetric-minified', '', false, true);
		wp_enqueue_script('umetric-minified', '', array(), false, true);
		wp_add_inline_script('umetric-minified', '/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a|select|input|button|textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);');
	}
