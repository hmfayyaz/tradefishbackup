...
class Iqonic_Extension_Elementor
{
    ...
    public function elementor_init()
    {
        Plugin::$instance->elements_manager->add_category(
            'iqonic-extension',
            [
                'title' => esc_html__('iqonic-extensions', 'iqonic'),
                'icon' => 'fa fa-plug',
            ]
        );
    }
        ...
        if (defined('ELEMENTOR_PATH') && class_exists('Elementor\Widget_Base')) {
            ...
            if (!empty($all_config)) {
                foreach ($all_config as $item) {
                    if (isset($item['class'])) {
                        Plugin::instance()->widgets_manager->register_widget_type(new $item['class']);
                    }
                }
            }
        }
            ...
            foreach ($elements as $item) {
                ...
                if (isset($config[$item]['dependency']['css'])) {
                    foreach ($config[$item]['dependency']['css'] as $css) {
                        ...
                        wp_enqueue_style($css['name'], plugin_dir_url(__FILE__) . $css['src'], array(), $this->version, true);
                    }
                }
            }
        }
