...
class Signature
{
	...
	public static function verify2($message, $signature, $coinbase)
	{
		...
		if (empty($signature) || empty($coinbase) || !is_string($signature) || !is_string($coinbase)) {
			...
			return [false, sprintf(__('Signature can not be verified for address "%3$s", message "%1$s" and signature "%2$s"', 'ethpress'), $message, $signature, $coinbase)];
		}
		if (self::_check_ecrecovers_with_php()) {
			...
			if (!is_string($address)) {
				...
				return [false, sprintf(__('Failed to verify signature locally for address "%3$s", message "%1$s" and signature "%2$s"', 'ethpress'), $message, $signature, $coinbase)];
			}
		} else {
			$address = self::extract_address_with_api($message, $signature);
			if (!is_string($address)) {
				Logger::log("verify failed for: $message, $signature, $coinbase: address is not a string");
				return [false, sprintf(__('Failed to verify signature remotely for address "%3$s", message "%1$s" and signature "%2$s"', 'ethpress'), $message, $signature, $coinbase)];
			}
		}
		if (strtolower($coinbase) !== strtolower($address)) {
			...
			return [false, sprintf(__('Failed to verify signature. The address "%4$s" extracted for address "%3$s", message "%1$s" and signature "%2$s"', 'ethpress'), $message, $signature, $coinbase, $address)];
		}
		return [true, null];
	}
