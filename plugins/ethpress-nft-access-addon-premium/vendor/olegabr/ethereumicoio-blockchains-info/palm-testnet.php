...
add_filter('ethereumico.io/supported-blockchains', function ($blockchains) {
    return array_merge($blockchains, array(
        "palm-testnet" => __('Palm Testnet', 'ethereum-wallet'),
    ));
}, 17, 1);

add_filter('ethereumico.io/supported-blockchains-info', function ($blockchains) {
    $info = apply_filters('ethereumico.io/blockchain-info', $blockchains && isset($blockchains['a11297108099']) ? $blockchains['a11297108099'] : null, 11297108099);
    return array_merge($blockchains, array(
        "palm-testnet" => $info,
        "a11297108099" => $info,
    ));
...
add_filter('ethereumico.io/blockchain-info/blockchain-id', function ($blockchainId, $blockchainName) {
    if (!in_array($blockchainName, ['palm-testnet'])) {
        return $blockchainId;
    }
    return 11297108099;
...
add_filter('ethereumico.io/blockchain-info', function ($info, $chainId) {
    ...
    $base_url = 'https://explorer.palm-uat.xyz/api?';
    ...
    return array_merge($info ? $info : array(), array(
        'blockchain_name' => 'palm-testnet',
        'blockchain_display_name' => 'Palm Testnet',
        ...
        'txhash_path_template' => 'https://explorer.palm-uat.xyz/tx/%s',
        'address_path_template' => 'https://explorer.palm-uat.xyz/address/%s',
