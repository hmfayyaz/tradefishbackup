...
add_filter('ethereumico.io/supported-blockchains', function ($blockchains) {
    return array_merge($blockchains, array(
        "optimism-mainnet" => __('Optimism Mainnet', 'ethereum-wallet'),
    ));
}, 22, 1);

add_filter('ethereumico.io/supported-blockchains-info', function ($blockchains) {
    $info = apply_filters('ethereumico.io/blockchain-info', $blockchains && isset($blockchains['a10']) ? $blockchains['a10'] : null, 10);
    return array_merge($blockchains, array(
        "optimism-mainnet" => $info,
        "a10" => $info,
    ));
...
add_filter('ethereumico.io/blockchain-info/blockchain-id', function ($blockchainId, $blockchainName) {
    if (!in_array($blockchainName, ['optimism-mainnet'])) {
        return $blockchainId;
    }
    return 10;
...
add_filter('ethereumico.io/blockchain-info', function ($info, $chainId) {
    ...
    return array_merge($info ? $info : array(), array(
        'blockchain_name' => 'optimism-mainnet',
        'blockchain_display_name' => 'Optimism',
        'interblock_period_seconds' => 24,
        'currency_ticker' => 'ETH',
        'currency_name' => 'Ether',
        ...
        'txhash_path_template' => 'https://optimistic.etherscan.io/tx/%s',
        'address_path_template' => 'https://optimistic.etherscan.io/address/%s',
        ...
        'base_explorer_display_url' => 'optimistic.etherscan.io',
        'explorer_api_url' => 'https://optimistic.etherscan.io/myapikey',
        'explorer_register_url' => 'https://optimistic.etherscan.io/register',
    ));
}, $ETHEREUMICOIO_BLOCKCHAINS_INFO_VERSION, 2);
