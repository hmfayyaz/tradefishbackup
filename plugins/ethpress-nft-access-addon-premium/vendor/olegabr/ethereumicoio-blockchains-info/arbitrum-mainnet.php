...
add_filter('ethereumico.io/supported-blockchains', function ($blockchains) {
    return array_merge($blockchains, array(
        "arbitrum-mainnet" => __('Arbitrum Mainnet', 'ethereum-wallet'),
    ));
}, 25, 1);

add_filter('ethereumico.io/supported-blockchains-info', function ($blockchains) {
    $info = apply_filters('ethereumico.io/blockchain-info', $blockchains && isset($blockchains['a42161']) ? $blockchains['a42161'] : null, 42161);
    return array_merge($blockchains, array(
        "arbitrum-mainnet" => $info,
        "a42161" => $info,
    ));
...
add_filter('ethereumico.io/blockchain-info/blockchain-id', function ($blockchainId, $blockchainName) {
    if (!in_array($blockchainName, ['arbitrum-mainnet'])) {
        return $blockchainId;
    }
    return 42161;
...
add_filter('ethereumico.io/blockchain-info', function ($info, $chainId) {
    ...
    return array_merge($info ? $info : array(), array(
        'blockchain_name' => 'arbitrum-mainnet',
        'blockchain_display_name' => 'Arbitrum',
        ...
        'txhash_path_template' => 'https://arbiscan.io/tx/%s',
        'address_path_template' => 'https://arbiscan.io/address/%s',
        ...
        'base_explorer_display_url' => 'arbiscan.io',
