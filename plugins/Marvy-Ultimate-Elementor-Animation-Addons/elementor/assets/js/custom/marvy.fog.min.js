(function( $ ) {
    ...
    var VisualFogAnimation = {
        initFog: function () {
            ...
            elementorFrontend.hooks.addAction('frontend/element_ready/container', VisualFogAnimation.initFogWidget);
        },
        initFogWidget: function ($scope) {
            ...
            if (window.isEditMode || window.elementorFrontend.isEditMode()) {
                ...
                $.each(editorElements.models, function (i, el) {
                    ...
                    if(Object.keys(fogAnimationArgs).length !== 0){
                        
                        if(fogAnimationArgs.__globals__ && fogAnimationArgs.__globals__.marvy_fog_animation_highlight_color && fogAnimationArgs.__globals__.marvy_fog_animation_highlight_color !== ""){
                            var highlightColor_id = fogAnimationArgs.__globals__.marvy_fog_animation_highlight_color.split('=')[1];
                            var highlightColor_arr = global_color.find(element => element._id  === highlightColor_id);
                            settings.highlightColor = highlightColor_arr.color; 
                        }
                        else{
                            settings.highlightColor = fogAnimationArgs.marvy_fog_animation_highlight_color;
                        }
                        if(fogAnimationArgs.__globals__ && fogAnimationArgs.__globals__.marvy_fog_animation_base_color && fogAnimationArgs.__globals__.marvy_fog_animation_base_color !== ""){
                            var baseColor_id = fogAnimationArgs.__globals__.marvy_fog_animation_base_color.split('=')[1];
                            var baseColor_arr = global_color.find(element => element._id  === baseColor_id);
                            settings.baseColor = baseColor_arr.color; 
                        }
                        else{
                            settings.baseColor = fogAnimationArgs.marvy_fog_animation_base_color;
                        }
                        if(fogAnimationArgs.__globals__ && fogAnimationArgs.__globals__.marvy_fog_animation_lowlight_color && fogAnimationArgs.__globals__.marvy_fog_animation_lowlight_color !== ""){
                            var lowlightColor_id = fogAnimationArgs.__globals__.marvy_fog_animation_lowlight_color.split('=')[1];
                            var lowlightColor_arr = global_color.find(element => element._id  === lowlightColor_id);
                            settings.lowlightColor = lowlightColor_arr.color; 
                        }
                        else{
                            settings.lowlightColor = fogAnimationArgs.marvy_fog_animation_lowlight_color;
                        }
                        if(fogAnimationArgs.__globals__ && fogAnimationArgs.__globals__.marvy_fog_animation_midtone_color && fogAnimationArgs.__globals__.marvy_fog_animation_midtone_color !== ""){
                            var midtoneColor_id = fogAnimationArgs.__globals__.marvy_fog_animation_midtone_color.split('=')[1];
                            var midtoneColor_arr = global_color.find(element => element._id  === midtoneColor_id);
                            settings.midtoneColor = midtoneColor_arr.color; 
                        }
                        else{
                            settings.midtoneColor = fogAnimationArgs.marvy_fog_animation_midtone_color;
                        }                      
    ...
    function fogAnimation(target,settings,sectionId) {
        ...
        if (checkElement.length >= 0) {
            ...
            var fogZindex = document.querySelector('.marvy-custom-fog-animation-section-'+sectionId+' .elementor-container , .marvy-custom-fog-animation-section-'+sectionId+'>*');
            fogZindex.style.zIndex = '99';

            // Set min height
            var fogMinHeight = document.querySelector(".elementor-element-"+sectionId);
            fogMinHeight.closest('.elementor-top-section,.e-con-boxed,.e-con-full').style.minHeight = "100px";
