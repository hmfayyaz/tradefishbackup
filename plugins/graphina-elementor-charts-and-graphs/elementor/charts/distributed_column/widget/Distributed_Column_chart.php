...
class Distributed_Column_chart extends Widget_Base
{
    ...
    protected function render()
    {
        ...
        if (isGraphinaPro() && $settings['iq_' . $type . '_chart_data_option'] !== 'manual') {
            ...
            if(empty($seriesCount)){
                $categoryList = [];
            }
            foreach ($categoryList as $v) {
                $data["category"][] = (string)graphina_get_dynamic_tag_data($v, 'iq_' . $type . '_chart_category');
            }
            $valueList = $settings['iq_' . $type . '_value_list_4_' . ($settings['iq_' . $type . '_can_chart_negative_values'] === 'yes' ? 2 : 1) . '_'];
            $value = [];
        ...
        if (isRestrictedAccess('column', $mainId, $settings, false) === false) {
            ?>
            <script>
                            ...
                            formatter: function (val) {
                                if('<?php echo !empty( $settings['iq_' . $type . '_chart_xaxis_label_show']) && $settings['iq_' . $type . '_chart_xaxis_label_show'] === 'yes' ?>'){
                                    if('<?php echo !empty($settings['iq_' . $type . '_is_chart_horizontal']) && $settings['iq_' . $type . '_is_chart_horizontal'] === 'yes' ?>'){
                                        let decimal = parseInt('<?php echo !empty($settings['iq_' . $type . '_chart_xaxis_prefix_postfix_decimal_point']) ? $settings['iq_' . $type . '_chart_xaxis_prefix_postfix_decimal_point'] : 0; ?>') || 0;
                                        if("<?php echo esc_html($settings['iq_' . $type . '_chart_xaxis_format_number']) === "yes"; ?>"){
                                            val = graphinNumberWithCommas(val,'<?php echo $localStringType ?>',decimal)
                                        }
                                        else if("<?php echo !empty($settings['iq_' . $type . '_chart_xaxis_label_pointer']) && esc_html($settings['iq_' . $type . '_chart_xaxis_label_pointer']) === 'yes'; ?>"
                                            &&  typeof graphinaAbbrNum  !== "undefined"){
                                            val = graphinaAbbrNum(val ,  parseInt("<?php echo esc_html($settings['iq_' . $type . '_chart_xaxis_label_pointer_number']); ?>") || 0 );
                                        }else{
                                            val = parseFloat(val).toFixed(decimal)
                                        }
                                        return '<?php echo esc_html($xLabelPrefix) ; ?>' + val + '<?php echo esc_html($xLabelPostfix); ?>';
                                    }else{
                                        val = '<?php echo esc_html($xLabelPrefix) ; ?>' + val + '<?php echo esc_html($xLabelPostfix); ?>'
                                        if(val){
                                            val = val.split(',')
                                        }
                                        return val;
